...
def movement():
    for ablock in block_sprite_list:
        if (player.rect.x <= ablock.rect.x + ablock.width and player.rect.x >= (ablock.rect.x - player.width)) or (player.rect.y <= ablock.rect.y + ablock.height and player.rect.y >= (ablock.rect.y - player.height)):
            return False
...
while not done:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and movement() == True:
